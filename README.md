...
PWA local-first para gastos, compras, proveedores, transferencias de mercancía, asistencias, pagos a colaboradores y cortes de caja de las tiendas.
...
12. Revisa antes de aplicar [`supabase/migrations/202609010001_half_shift_attendance.sql`](supabase/migrations/202609010001_half_shift_attendance.sql), después de las migraciones de capacidades de operadores. Agrega `attendance_type`, conserva presentes históricos como turnos completos, congela el tipo en los pagos y actualiza la RPC autoritativa de asistencia y pagos.
13. Adapta y ejecuta [`supabase/setup-operations.example.sql`](supabase/setup-operations.example.sql) para crear tiendas y asignar cada cajera.
14. Configura `VITE_SUPABASE_URL` y `VITE_SUPABASE_PUBLISHABLE_KEY` en `.env.local`.
15. Verifica que cada perfil `cashier` tenga `store_id`; una cajera sin tienda no tendrá acceso a datos operativos por RLS.
