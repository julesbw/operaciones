...
Dexie v9 agrega `appContexts`. El registro `current` conserva únicamente el perfil mínimo para configurar la experiencia local: identificador, nombre, rol, tienda y fechas de autenticación/sincronización. No contiene JWT, credenciales ni permisos remotos. Dexie v10 agrega `payments`, `paymentAttendanceItems` y `compensationHistory`, además de la selección de pagos en los borradores de Corte. Dexie v11 agrega `exportCandidates` y `exportBatches` para consulta cacheada. Dexie v12 agrega `centralCashSummary`, `centralCashMovements` y `centralCashPendingClosings`. Dexie v13 agrega `suppliers`, `purchases` y `purchasePayments`, además de la selección de Compras en los borradores de Corte. Dexie v14 agrega `closingAdjustments` para consultar offline el historial append-only de ajustes de Cortes cerrados. Estas tablas administrativas se limpian si el perfil autenticado deja de ser administrador.

Una inicialización se considera completa después de obtener el perfil y las referencias autorizadas, verificar el app shell en producción y guardar `LocalAppContext`. Vite genera un `RELEASE_ID` por build e inyecta en el service worker la lista completa de assets emitidos, incluidos todos los chunks de imports dinámicos. La instalación usa `cache.addAll` de forma atómica; `offlineShellService` sólo acepta el precache completo del mismo release que ejecuta el cliente. Las respuestas de Supabase no forman parte de esa caché.

Las páginas de Gastos, Transferencias, Colaboradores/Asistencias, Compras, Cortes, Caja Central, Exportación y Ajustes se cargan con imports dinámicos. Asistencias permanece en el chunk de Colaboradores; Pagos tiene un chunk propio y sólo se importa al abrir su pestaña con la capability `payments`. Un único `Suspense` muestra el fallback compartido. Si un chunk falla, el límite de error reconoce los mensajes de Chrome, Safari y Firefox, compara el release del cliente con el worker activo y permite como máximo una recarga automática por transición registrada en `sessionStorage`; los errores normales de renderizado no disparan recargas automáticas.
